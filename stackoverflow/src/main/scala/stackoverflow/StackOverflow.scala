...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] =
    ...
    val questions = cached.filter(_.postingType == 1).map(p => p.id -> p)
    val answers   = cached.filter(_.postingType == 2).flatMap(p => p.parentId.map(qid => qid -> p))
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    grouped.values.flatMap { iterable =>
      iterable.headOption.map { (q, _) => q -> answerHighScore(iterable.map(_._2).toArray) }
    }

  /** Compute the vectors for the kmeans */
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] =
    /** Return optional index of first language that occurs in `tags`. */
    def firstLangInTag(tag: Option[String], ls: List[String]): Option[LangIndex] =
      tag match
        case None       => None
        case Some(lang) =>
    ...
    scored.flatMap { (q, score) => firstLangInTag(q.tags, langs).map { idx => idx * langSpread -> score } }.cache()
  ...
  def clusterResults(
    ...
    val median = closestGrouped.mapValues { vs =>
      val mostCommonLang      = vs.groupBy(_._1).map((k, v) => k / langSpread -> v.size).maxBy(_._2)
      // most common language in the cluster
      val langLabel: String   = langs(mostCommonLang._1)
      // percent of the questions in the most common language
      val langPercent: Double = 100 * mostCommonLang._2 / vs.size
