...
object WikipediaRanking extends WikipediaRankingInterface:
  ...
  val sc: SparkContext = new SparkContext("local[4]", "wiki-app", conf)
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd
      .map(w => if (w.mentionsLanguage(lang)) 1 else 0)
      .aggregate(0)((acc, w) => acc + w, _ + _)
  ...
  def makeIndex(
    ...
    sc.parallelize(langs)
      .cartesian(rdd)
      .filter { case (lang, w) => w.mentionsLanguage(lang) }
      .groupByKey()
