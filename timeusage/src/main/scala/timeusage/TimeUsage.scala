...
object TimeUsage extends TimeUsageInterface:
  ...
  def classifiedColumns(
    ...
    // format: off
    val primaryNeeds = Array("t01", "t03", "t11", "t1801", "t1803")
    val workingActivities = Array("t05", "t1805")
    val leisureActivities = Array("t02", "t04", "t06", "t07", "t08", "t09",  "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    // format: on

    columnNames.foldRight((List[Column](), List[Column](), List[Column]())) {
      case (name, (primary, working, leisure)) =>
        if primaryNeeds.exists(name.startsWith) then
          (col(name) :: primary, working, leisure)
        else if workingActivities.exists(name.startsWith) then
          (primary, col(name) :: working, leisure)
        else if leisureActivities.exists(name.startsWith) then
          (primary, working, col(name) :: leisure)
        else (primary, working, leisure)
    }
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = when($"teage" <= 22, "young").when($"teage" > 55, "elder").otherwise("active").as("age")
